 ...
 * @author JKRANES
 */
public class JpdlJoinNodeBuilder extends JpdlNodeBuilder<Process.Join, Activities> {

    public JpdlJoinNodeBuilder(ProcessContext context, Activities activities) {
        super(context, new Process.Join(), activities);
    }

    @Override
    protected void buildInternal() {
        getNode().setName(getContext().generateId("join"));
        Activities source = getActivity();
        if (source.getMergeCondition() != null ) {
            getNode().setMultiplicity(source.getMergeCondition());
        }
    }
