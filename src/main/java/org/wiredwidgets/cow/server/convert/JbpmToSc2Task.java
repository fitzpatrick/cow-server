 ...
 * @author JKRANES
 */
public class JbpmToSc2Task extends AbstractConverter implements Converter<org.jbpm.api.task.Task, Task> {
    ...
    @Override
    public Task convert(org.jbpm.api.task.Task source) {
        ...
        if (source.getDuedate() != null) {
            target.setDueDate(this.getConverter().convert(source.getDuedate(), XMLGregorianCalendar.class));
        }
